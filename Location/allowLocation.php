    ...
    if (isset($_COOKIE['lat']) && (isset($_COOKIE['long']))) 
    {
        ...
        if(isset($_SESSION['member_email']))
        {
            ...
            if($fetch_from_latlong->rowCount() > 0)
            {
                ...
                $url="http://localhost/Blood/Location/nearestRequester.php?mlat=$elat&mlong=$elong";
                // header("location:$url");
            }
            else
            {
        ...
        else
        {
            $elat=base64_encode($lat);
            $elong=base64_encode($long);
            $url="http://localhost/Blood/Location/nearestRequester.php?lat=$elat&long=$elong";
            // header("location:$url");
        }
    }   
}
